...
export function MedicineListwithSales() {
  ...
  const deleteMedicine = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8001/admin/availableMedicines/${id}`);
        ...
        <VStack align="start" w="100%">
          {filteredMedicines.map((medicine, index) => (
            <Box key={medicine._id} w='100%' p={4} borderWidth="10px" borderRadius="md">
              ...
              <Button size="sm" colorScheme="teal" onClick={() => openEditModal(medicine)} mr={2}>Edit</Button>
              <Button size="sm" colorScheme="red" onClick={() => deleteMedicine(medicine._id)}>Delete</Button>
